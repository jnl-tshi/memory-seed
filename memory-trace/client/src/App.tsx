...
// Hover explainers for the edge-filter chips: each toggle shows or hides one
// KIND of line over the already-laid-out graph (a visibility filter, not a
// re-query - see edgeTypesForScope). The text says what the line means, so a
// reader knows what a chip is turning on before they click it.
const EDGE_DESCRIPTIONS: Record<RendererGraphEdge["edge_type"], string> = {
  related: "Show or hide associative links an author noted between two entries — related, not a lifecycle change.",
  replaces: "Show or hide lines where a later entry retired an earlier decision.",
  evolves: "Show or hide lines where a later entry refined an earlier decision that still holds.",
  branch: "Show or hide fork-and-merge connections between entries on the same git branch.",
  topic: "Show or hide links between entries that share a controlled-vocabulary topic.",
  agent: "Show or hide links between entries written by the same agent.",
  day: "Show or hide links between entries logged on the same day.",
};
...
// shape as the trail settings above, for the same reason - one key, validated
// per field, defaults on anything unparseable.
...
export default function App() {
  ...
  // starting step so there is always at least one page on screen. Shrinking
  // makes `previousOverview` (recorded for the exhaustion test above) stale
  // in the growth direction it was captured for, so it is cleared rather than
  // carried forward — the next "Show more" re-establishes it from scratch.
  async function showLessOverview() {
