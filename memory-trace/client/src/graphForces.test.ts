...
import { DEFAULT_FORCES, forceParameters, readForceSettings } from "./graphForces.ts";

test("sliders map to sane force units at both ends", () => {
  const low = forceParameters({ centre: 0, repel: 0, linkForce: 0, linkDistance: 0 });
  const high = forceParameters({ centre: 1, repel: 1, linkForce: 1, linkDistance: 1 });
  ...
  assert.ok(low.linkDistance >= 25 && high.linkDistance <= 300);
});

test("every force is monotonic in its slider", () => {
  const at = (value: number) => forceParameters({ centre: value, repel: value, linkForce: value, linkDistance: value });
  for (let step = 0; step < 10; step += 1) {
    ...
    assert.ok(b.linkDistance > a.linkDistance);
  }
});
...
test("defaults reproduce the settle-only layout's character", () => {
  ...
  assert.equal(Math.round(params.linkDistance), 163);
