...
A link sidecar records `replaces`/`evolves`/`related_entries` edges authored
...
skeleton - if `graph()` emits a sidecar-declared `replaces` edge, the whole
...
class LinkSidecarReadPathTests(unittest.TestCase):
    ...
    def _graph_edges(self, app):
        ...
        payload = TestClient(app).get(
            "/api/graph", params={"granularity": "entry", "edge_types": "replaces,evolves,related"}
        ).json()
    ...
    def test_reader_parses_edges_keyed_to_source_entry(self):
        self._write_sidecar(
            _link_block("2026-06-01 10:00", NEW, "New palette", replaces=[OLD], evolves=[THIRD])
        )
        parsed = entry_link_sidecars(self.cwd)
        self.assertEqual(parsed[NEW]["replaces"], (OLD,))
        self.assertEqual(parsed[NEW]["evolves"], (THIRD,))

    def test_reader_unions_multiple_blocks_for_one_entry(self):
        self._write_sidecar(
            _link_block("2026-06-01 10:00", NEW, "a", replaces=[OLD])
            + _link_block("2026-06-01 10:00", NEW, "b", replaces=[THIRD])
        )
        self.assertEqual(set(entry_link_sidecars(self.cwd)[NEW]["replaces"]), {OLD, THIRD})

    def test_graph_emits_sidecar_replaces_edge(self):
        # The walking skeleton: a sidecar-declared replaces must show up as a
        # graph edge the Trail can draw.
        self._write_sidecar(_link_block("2026-06-01 10:00", NEW, "New palette", replaces=[OLD]))
        edges = self._graph_edges(self._app())
        self.assertIn({"source": NEW, "target": OLD, "type": "replaces"}, edges)

    def test_v1_graph_inherits_sidecar_lifecycle_edges(self):
        # Sidecar edges are ordinary replaces/evolves entries in the edges
    ...
    def test_graph_has_no_lifecycle_edge_without_sidecar(self):
        edges = self._graph_edges(self._app())
        self.assertFalse([e for e in edges if e["type"] in {"replaces", "evolves"}])

    def test_self_reference_is_dropped(self):
        self._write_sidecar(_link_block("2026-06-01 10:00", NEW, "self", replaces=[NEW, OLD]))
        edges = self._graph_edges(self._app())
        lifecycle = [e for e in edges if e["type"] == "replaces"]
        self.assertEqual(lifecycle, [{"source": NEW, "target": OLD, "type": "replaces"}])
    ...
    def test_malformed_sidecar_filename_is_flagged_not_read(self):
        ...
        (links_dir / "not-a-date.md").write_text(
            _link_block("2026-06-01 10:00", NEW, "x", replaces=[OLD]), encoding="utf-8"
        )
