...
- `memory-seed session append --title ... --user-initials ... --agent-type ... [--decisions-file d.json] [--body-file f]` appends a session entry with structure enforced: target resolution, heading timestamp (refusing out-of-order appends loudly), canonical deterministic `entry_id`, YAML shape, ref validation (fabricated or forward-pointing ids refused), controlled-topic resolution, and git branch auto-capture. Topics and lifecycle links (`replaces`/`evolves`/`related_entries`) arrive per decision through `--decisions-file` and are written to the topic and link sidecars; the entry-level `--topics`/`--replaces`/`--evolves`/`--related` flags are refused as of 2026-08-09 (see CHANGELOG). The body prose passes through verbatim. Auto-capture reads the HEAD of the working tree that owns the memory dir, so **pass `--branch <name>` explicitly when several sessions may share one working tree** (their HEAD is identical and a session's own branch is never passed to the CLI, so nothing can tell them apart), or `--no-branch` to omit the field rather than record a value you cannot vouch for; reasoning and the standing harness convention are in `.memory-seed/skills/session_logging.md`.
