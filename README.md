...
- **Inspectable decisions and documentation, not vague summaries.** Typed DRAFTS records separate governing decisions from small-work evidence, with explicit scope, decision disposition/reason, sources, lifecycle edges, artifact lineage, and commit links - validated by `links check`.
...
| GitHub Copilot CLI / cloud agent | `.github/hooks/memory-seed.json` | `agentStop` | Included in startup context | `sessionStart` (command) |

The lifecycle hooks are cross-platform Python scripts in `.memory-seed/hooks/`:
...
- `memory-retrieval-check.py` — before substantive work, reminds the agent to use `memory_search` for topical recall and the SessionStart/`situate` context route for current state. Gated by an 8-hour marker file so it fires about once per working session. Claude receives model context; VS Code receives the same text as a user-visible warning because `UserPromptSubmit` supports only the common output shape there.
- `session-start-context.py` - at session start, directs the agent through the nearest `AGENTS.md` and `orientation.md`, then consumes the same deterministic report as `memory-seed situate`. It injects checkout/git/integration facts plus the latest applicable session file's path, character/byte/entry counts, and a mechanical route: read the whole file directly at or below 12,000 characters; above that, use a read-only economy worker to compress the whole file into a source-linked briefing of at most 800 tokens. It never injects entry bodies or invokes a model. With a configured user it selects that user's applicable file and lists same-day co-contributor files without loading their content. Copilot CLI and cloud agents use the command hook too, so orientation also runs for resumed and non-interactive sessions where prompt hooks do not fire.
- `file-touch-decisions.py` - after supported edit tools change a file named by a decision's `F:` reference, injects the recorded decision and its supersession status before the turn finishes. It accepts Claude, VS Code, and Copilot CLI payload names, including multi-file VS Code edits. Copilot command hooks detect and skip VS Code's converted replay so the editor processes each event once through `.claude/settings.json`.
...
The hooks never block file edits. A turn-end hook may ask the agent to continue long enough to record required session memory, with `stop_hook_active` preventing repeat-continuation loops. The scripts use Python 3.11+, which Memory Seed already requires.
...
- `memory-seed session fuse --branch <branch> [--base <ref>] [--apply]` dry-runs or applies branch-local session entries and sidecars into the current integration tree. It accepts ordinary entries only when `branch:` matches the source branch; an aggregate may carry an already-fused child entry unchanged only through a recursive, bounded carrier proof: each two-parent non-first-parent merge has one final `Memory-Entry` receipt and the exact record remains continuous on that aggregate's own first-parent segment. The declared child must remain a current local branch ref; if it was deleted or renamed, restore that exact ref and retry instead of editing authorship. It blocks missing, copied, altered, reintroduced, ambiguous, unreceipted, octopus, or otherwise mismatched provenance; missing/duplicate `entry_id` values; edits to existing entries or sidecars; non-chronological targets; and sidecars without a parent entry already on the base branch or accepted for promotion in the same fuse. `--apply` requires an in-progress merge and normalizes imported files to the grouped layout.
- `memory-seed session merge-branch --branch <branch> [--dry-run]` wraps the whole integration dance in one command: fuse dry-run gate, `git merge --no-ff --no-commit`, session-path reset to base content, fuse apply, stage, merge commit, and source-worktree cleanup. Cleanup first uses Git; if Git deregisters the exact proven clean checkout but leaves directory residue, Memory Seed revalidates its repository, branch, administrative pointer, directory identity, and path type before deleting only that residue. Dirty, locked, still-registered, replaced, or otherwise unproven paths are retained, and a pending cleanup returns a distinct non-success result even though the merge remains committed. Fuse issues abort before the merge ever starts; a refusal after that point auto-runs `merge --abort` on its own; only a genuine non-session content conflict is left in progress for manual resolution, as is a post-fuse commit failure; session files always land timestamp-sorted regardless of how git would have line-merged them. Requires a clean working tree.
...
- `memory-seed session append --title ... --user-initials ... --agent-type ... [--decisions-file d.json] [--body-file f]` appends a session entry with structure enforced: target resolution, heading timestamp (refusing out-of-order appends loudly), canonical deterministic `entry_id`, YAML shape, typed `### Records` validation, ref validation (fabricated or forward-pointing ids refused), controlled-topic resolution, git branch auto-capture, and the same content-bound ADR review preflight as MCP `memory_session_append`. Decision records require `D`, nested `Scope` and `Disposition`, and their own `R`; Documentation records require `D` plus nested `Scope`, remain searchable, and cannot own `replaces`/`evolves` or ADR authority. The `--decisions-file` name is retained for compatibility but carries one object per body record. A lifecycle-linked first call returns the full ADR contexts and a receipt while writing zero bytes; put the outcomes in that envelope and retry with `--adr-review-receipt`. MCP record objects also require `origin: user | agent`; supplied origins are generated as entry metadata and missing origins on historical entries remain valid. The body prose passes through verbatim. Auto-capture reads the HEAD of the working tree that owns the memory dir, so **pass `--branch <name>` explicitly when several sessions may share one working tree** (their HEAD is identical and a session's own branch is never passed to the CLI, so nothing can tell them apart), or `--no-branch` to omit the field rather than record a value you cannot vouch for; reasoning and the standing harness convention are in `.memory-seed/skills/session_logging.md`.
- `memory-seed adr reviewed --adr-id <id> --entry <existing-entry-id> --reason <text>` (MCP twin: `memory_adr_reviewed`) records an evidence-backed reviewed-no-change event without creating a pretextual session entry or moving the ADR head. Both surfaces use one validator and append-only writer.
...
**Auto-registration:** `memory-seed init` and `memory-seed update` automatically register `uvx --from memory-seed memory-seed-mcp --stdio` in each supported vendor's MCP config — `.mcp.json` at the project root (Claude Code and portable Agent Host configuration), `.cursor/mcp.json` (Cursor), `.gemini/settings.json` (Gemini CLI), `.codex/config.toml` (Codex CLI), `.github/mcp.json` (Copilot CLI), and `.vscode/mcp.json` (VS Code). No manual config is needed for projects initialised with Memory Seed. The `uvx --from` form is used so the command works regardless of whether `~/.local/bin` is on the agent's PATH.
...
> **Copilot and VS Code can discover more than one project MCP file.** Copilot CLI gives a same-directory `.mcp.json` definition precedence over `.github/mcp.json`. VS Code uses `.vscode/mcp.json` locally and can forward it to Agent Host, while Agent Host also reads workspace `.mcp.json` natively. Generated projects point all of these entries at the same packaged `uvx` command. This source repository deliberately makes root `.mcp.json` run the checkout with `uv run --no-sync`, so local development wins over the packaged definitions. Confirm the selected server with `copilot mcp get memory-seed` or VS Code's **MCP: List Servers**, then invoke `memory_search` once after changing configuration.
