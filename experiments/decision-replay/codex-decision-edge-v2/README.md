...
```powershell
python experiments/decision-replay/codex-decision-edge-v2/prepare.py `
  --seed 20260816 `
...
python -m unittest discover `
  -s experiments/decision-replay/codex-decision-edge-v2/tests `
  -p "test_*.py"

python experiments/decision-replay/codex-decision-edge-v2/verify_harness.py
```
...
the pre-freeze reason and immutable commit chain are recorded in `INSTRUMENT-AMENDMENTS.md`.

## Grade a candidate

```powershell
python experiments/decision-replay/codex-decision-edge-v2/grade.py `
...
The schema-v4 JSON reports separate decision-row, two-way focus, non-projection equality, entry-ref,
invalid-ordinal, singular-`d1`, public-test, scope, and candidate-test gates. The hidden oracle lives
...
failures/errors. Printed or fabricated unittest transcripts are never scoring evidence. The runner's
SHA-256 and version are included in each grade.
...
`python -m unittest memory-trace/tests/test_trail_decision_edges.py`. That module is intentionally absent
