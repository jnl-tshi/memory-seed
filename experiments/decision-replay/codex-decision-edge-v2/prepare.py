...
PUBLIC_TEST_COMMAND = "python -m unittest memory-trace/tests/test_trail_decision_edges.py"
SUBJECT_PROMPT = (
...
def _context_receipt(seed: int, run_id: str, subject_id: str) -> str:
    payload = f"codex-decision-edge-v2:{seed}:{run_id}:{subject_id}:receipt".encode()
    return "ctx_" + hashlib.sha256(payload).hexdigest()[:24]


def _subject_id(seed: int, run_id: str, block_number: int, slot: int) -> str:
    payload = f"codex-decision-edge-v2:{seed}:{run_id}:{block_number}:{slot}".encode()
...
def prepare_study(
    ...
    *,
    ...
    destination = (
        ...
        else (HERE / "runs" / f"{run_id}-artifacts").resolve()
    )
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="caller-owned new directory; default is gitignored runs/<run-id>-artifacts",
    )
