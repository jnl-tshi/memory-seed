"""Shared, standard-library-only helpers for the Codex v5 explicit-retrieval replay."""
...
ARMS = ("task-only-control", "prescribed-memory-retrieval")
BLOCK_COUNT = 8
...
MEMORY_RETRIEVAL_RUNNER_PATH = Path("RUN_MEMORY_RETRIEVAL.py")
FIXTURE_TEST_RUNNER = """\
...
"""
MEMORY_RETRIEVAL_RUNNER = """\
\"\"\"Fixture-local adapter for Memory Seed's public read-only retrieval API.\"\"\"
...
def main() -> int:
    ...
    fetch.add_argument("--chunk-id", required=True)
    ...
    if args.command == "search":
        payload = search_memory(
            ...
            # The historical fixture's public API exposes decision sections through
            # its supported ``section`` granularity.  Keep the treatment bound to
            # that real returned chunk ID rather than inventing a newer alias.
            granularity="section",
        )
    else:
        payload = get_chunk(args.chunk_id, ROOT)
...
The orchestration layer, not the candidate fixture, must capture a `pre_coding_receipt` before any
candidate edits. It records the supplied context receipt, evidence IDs (or `none`), rationale
propositions (or `none`), and an acknowledgement. The receipt is supplied later to `grade.py` in the
protocol JSON; do not create it as a candidate working-tree write.
"""
...
def sanitize_source(root: Path) -> dict[str, Any]:
    ...
    retrieval_runner_bytes = MEMORY_RETRIEVAL_RUNNER.encode("utf-8")
    (root / MEMORY_RETRIEVAL_RUNNER_PATH).write_bytes(retrieval_runner_bytes)
    transformations.append(
        {
            "path": MEMORY_RETRIEVAL_RUNNER_PATH.as_posix(),
            "action": "add-fixture-local-memory-retrieval-runner",
            "original_sha256": None,
            "transformed_sha256": sha256_bytes(retrieval_runner_bytes),
            "removed_lines": [],
        }
    )
