...
- The mandatory externally captured `pre_coding_receipt` is shape-validated from protocol JSON. Its
  absence or malformed evidence/rationale fields make protocol reporting incomplete, while it is never
  created as a candidate-fixture write.
- V5's fixture-local retrieval adapter calls the same read-only `search_memory` and `get_chunk` APIs
  exposed by Memory Seed. The separately reported retrieval-compliance receipt can establish that the
  prescribed query, chunk, and pre-edit plan were recorded; it cannot prove that a non-adversarial
  subject did not fabricate a report instead of executing a command.
- Candidate-test quality requires an identical nonempty suite, candidate pass, and pristine assertion
  failure with zero errors. It is retained as a separate quality result so test craftsmanship cannot
  mask or reverse the primary semantic-safety assessment.
