...
VALIDITY_GATES = ("public_task_tests", "bounded_scope")
CANDIDATE_TEST_QUALITY_GATE = "candidate_test_discriminates"
ALL_GATES = (*SEMANTIC_GATES, *VALIDITY_GATES, CANDIDATE_TEST_QUALITY_GATE)
PUBLIC_COMMAND = (sys.executable, FIXTURE_TEST_RUNNER_PATH.as_posix())
SCHEMA_VERSION = 7
GRADER_VERSION = "codex-decision-edge-grader-v7"
ORACLE_SCHEMA_VERSION = 1
ORACLE_VERSION = "codex-decision-edge-v5-semantic-v1"
...
PRE_CODING_RECEIPT_FIELDS = frozenset(
    {
        "context_receipt",
        "evidence_ids_used",
        "rationale_propositions_used",
...
RETRIEVAL_RECEIPT_FIELDS = frozenset(
    {"required", "query", "selected_chunk_id", "pre_edit_plan"}
)
PRE_EDIT_PLAN_FIELDS = frozenset(
    {"invariant", "affected_code_path", "acceptance_cases", "non_goals"}
)
REQUIRED_RETRIEVAL_QUERY = "decision edge never projected entry level"
REQUIRED_RETRIEVAL_CHUNK_ID = (
    "mse_j55kt6mq230zj2p4#decisions/"
...
def _valid_receipt_evidence(value: Any) -> bool:
    """Accept a nonempty evidence-ID list or the explicit no-evidence marker."""
...
def assess_retrieval_compliance(payload: Any, expected_required: bool | None) -> dict[str, Any]:
    """Assess the instructed retrieval workflow independently of patch correctness."""

    base = {
        "status": "not_assessed",
        "required": expected_required,
        "detail": "retrieval requirement was not supplied to the grader",
    }
    if expected_required is None:
        return base
    if not isinstance(payload, dict) or not isinstance(payload.get("retrieval_receipt"), dict):
        return {**base, "status": "fail", "detail": "retrieval receipt is absent or malformed"}
    receipt = payload["retrieval_receipt"]
    if set(receipt) != RETRIEVAL_RECEIPT_FIELDS:
        return {**base, "status": "fail", "detail": "retrieval receipt has an unexpected shape"}
    plan = receipt.get("pre_edit_plan")
    if not isinstance(plan, dict) or set(plan) != PRE_EDIT_PLAN_FIELDS:
        return {**base, "status": "fail", "detail": "pre-edit plan has an unexpected shape"}
    if not all(isinstance(plan.get(name), str) and plan[name].strip() for name in ("invariant", "affected_code_path")):
        return {**base, "status": "fail", "detail": "pre-edit plan lacks invariant or affected code path"}
    for name in ("acceptance_cases", "non_goals"):
        value = plan.get(name)
        if not isinstance(value, list) or not value or any(not isinstance(item, str) or not item.strip() for item in value):
            return {**base, "status": "fail", "detail": f"pre-edit plan has invalid {name}"}
    if expected_required:
        passed = (
            receipt.get("required") is True
            and receipt.get("query") == REQUIRED_RETRIEVAL_QUERY
            and receipt.get("selected_chunk_id") == REQUIRED_RETRIEVAL_CHUNK_ID
        )
        detail = "prescribed retrieval and pre-edit plan recorded" if passed else "required retrieval query or selected chunk does not match"
    else:
        passed = (
            receipt.get("required") is False
            and receipt.get("query") == "not-required"
            and receipt.get("selected_chunk_id") == "not-required"
        )
        detail = "task-only planning control recorded" if passed else "control receipt unexpectedly claims retrieval"
    return {**base, "status": "pass" if passed else "fail", "detail": detail}


def assess_protocol(payload: Any) -> dict[str, Any]:
    """Assess final evidence and the externally captured pre-coding receipt."""
    ...
    receipt = payload.get("pre_coding_receipt")
    if "pre_coding_receipt" in payload and not isinstance(receipt, dict):
        wrong_shapes.append("pre_coding_receipt")
    retrieval_receipt = payload.get("retrieval_receipt")
    if "retrieval_receipt" in payload and not isinstance(retrieval_receipt, dict):
        wrong_shapes.append("retrieval_receipt")
    invalid: list[str] = []
    ...
    if isinstance(validation, list):
        ...
        for validation_item in validation:
            if not isinstance(validation_item, dict) or set(validation_item) != {"command", "result"}:
                invalid.append("validation")
                break
            if any(
                not isinstance(validation_item[key], str) or not validation_item[key].strip()
                for key in ("command", "result")
            ):
                invalid.append("validation")
                break
    if isinstance(receipt, dict):
        if set(receipt) != PRE_CODING_RECEIPT_FIELDS:
            invalid.append("pre_coding_receipt")
        for name in ("context_receipt", "acknowledgement"):
            if not isinstance(receipt.get(name), str) or not receipt[name].strip():
                invalid.append("pre_coding_receipt")
        for name in ("evidence_ids_used", "rationale_propositions_used"):
            if not _valid_receipt_evidence(receipt.get(name)):
                invalid.append("pre_coding_receipt")
    if isinstance(retrieval_receipt, dict):
        if set(retrieval_receipt) != RETRIEVAL_RECEIPT_FIELDS:
            invalid.append("retrieval_receipt")
        plan = retrieval_receipt.get("pre_edit_plan")
        if not isinstance(retrieval_receipt.get("required"), bool):
            invalid.append("retrieval_receipt")
        if not isinstance(retrieval_receipt.get("query"), str) or not retrieval_receipt["query"].strip():
            invalid.append("retrieval_receipt")
        if not isinstance(retrieval_receipt.get("selected_chunk_id"), str) or not retrieval_receipt["selected_chunk_id"].strip():
            invalid.append("retrieval_receipt")
        if not isinstance(plan, dict) or set(plan) != PRE_EDIT_PLAN_FIELDS:
            invalid.append("retrieval_receipt")
...
def validate_report_schema(report: dict[str, Any]) -> None:
    ...
    retrieval = report["retrieval_compliance"]
    if not isinstance(retrieval, dict) or set(retrieval) != {"status", "required", "detail"}:
        raise ValueError("invalid retrieval-compliance shape")
    if retrieval["status"] not in {"pass", "fail", "not_assessed"}:
        raise ValueError("invalid retrieval-compliance status")
    if retrieval["required"] not in {True, False, None} or not isinstance(retrieval["detail"], str):
        raise ValueError("invalid retrieval-compliance values")
...
def _error_report(
    ...
    report = {
        ...
        "retrieval_compliance": assess_retrieval_compliance(protocol_payload, expected_retrieval),
    }
...
def grade_candidate(
    ...
    *,
    ...
    if baseline["status"] != "pass":
        return _error_report(candidate, baseline, protocol_payload, expected_retrieval)
    ...
    semantic_safety = all(gates[name]["status"] == "pass" for name in SEMANTIC_GATES)
    validity = all(gates[name]["status"] == "pass" for name in VALIDITY_GATES)
    ...
    report = {
        ...
        "grader_version": GRADER_VERSION,
        "instrument": "codex-decision-edge-v5",
        "status": "error" if operational_error else ("pass" if semantic_safety and validity else "fail"),
        ...
        "outcomes": {
            "semantic_safety_pass": semantic_safety,
            ...
            "candidate_test_quality": gates["candidate_test_discriminates"]["status"] == "pass",
        },
        "protocol": assess_protocol(protocol_payload),
        "retrieval_compliance": assess_retrieval_compliance(protocol_payload, expected_retrieval),
    }
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="sealed orchestration expectation; scored separately from semantic correctness",
    )
    ...
    report = grade_candidate(
        ...
        expected_retrieval=(
            None
            if args.expected_retrieval is None
            else args.expected_retrieval == "required"
        ),
    )
