...
def _provenance_topology(
    ...
    A caller-provided owner is an *inspection selector*, never authority.  An
    active pod is valid only when its runtime path names an actual nested
    runtime, its id equals that runtime's final path component, and (for a
    write) the invoking cwd resolves to that same nested runtime.
    """
    ...
    # A Git worktree is an independent checkout, not a nested pod merely
    # because its filesystem location happens to sit under another checkout.
    if not (active.workspace_root / ".git").exists():
        ...
        for candidate in (parent, *parent.parents):
            if (candidate / ".memory-seed").is_dir():
                ...
                break
    active_path = (
        active.workspace_root.relative_to(outer).as_posix()
        if active.workspace_root != outer else "."
    )
    runtime = (
        normalize_runtime_ownership(owner)
        if owner is not None
        else build_runtime_ownership(
    ...
    # Retired/detached records may remain after their runtime directory is
    # gone. They can be read only, and their existing sidecar remains the
    # evidence; no selector can mint a writable retired identity.
...
def _git_sidecar_baseline(root: Path, path: Path) -> dict[str, Any]:
    """Read one Git baseline with the shared unavailable-Git semantics.

    This is an audit input, so a missing executable/repository is evidence the
    append-only claim cannot be proved, not an adapter exception.
    """
...
def _append_only_status(root: Path, path: Path) -> dict[str, Any]:
    """Mechanically anchor event order to the committed Git prefix when present."""
    ...
    if current.startswith(baseline_text):
        try:
            relative = path.resolve().relative_to(root.resolve()).as_posix()
...
def provenance_surface(
    ...
    try:
        ...
        path = _provenance_sidecar(root, runtime)
        return {
            "ok": False, "action": "check", "path": str(path), "runtime": runtime,
