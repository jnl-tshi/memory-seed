...
inbox -> todo -> completed     (shipped / applied)
              ...
              -> superseded    (replaced; points to its successor)
...
- This repository uses the numbered taxonomy with promoted terminal lanes: `docs/1_Inbox/`,
  ...
  `docs/5_Completed/`, `docs/6_Rejected/`, `docs/7_Superseded/`, `docs/8_Deferred/`.
...
superseded=replaced, deferred=parked); spec = live normative contracts (candidates in `draft/`);
...
3. **Route each outcome to its own lane — do not dump everything in "completed".** Shipped -> completed;
   declined -> rejected (kept, with a one-line reason); replaced -> superseded (with a pointer to the
...
8. **Never `git rm` a rejected or superseded document — move it to its lane.** Deletion loses the why-not
   and the reasoning trail (mirrors the memory system's supersede-don't-delete rule).
...
`blocked_by` (todo); `superseded_by` / `split_into`, `rejected_reason`, `extracted_into` (terminal /
...
Dependencies, Acceptance criteria. Use explicit dates when marking shipped, rejected, or superseded work.
...
(-> rejected); it was superseded by a newer canonical plan (-> superseded, with a pointer); it is a
