...
- When a `.memory-seed/` runtime exists, each present entry-point file (`AGENTS.md`/`CLAUDE.md`/`GEMINI.md`/`.github/copilot-instructions.md`) routes into it â€” either ours (has `memory-system-version` frontmatter) or a foreign file carrying our `<!-- BEGIN memory-seed -->` block. A foreign file with no block is an orphaned runtime (warned, non-fatal); run `memory-seed update` to inject the block.
