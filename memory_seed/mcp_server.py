    ...
    },
    {
        ...
        "inputSchema": {
            ...
            "properties": {
                ...
                "replaces": {"type": "array", "items": {"type": "string"}, "description": "entry_id values this entry retires (the old decision is now wrong or dead). Granularity is mandated (2026-07-24): a target with decisions must be narrowed as <entry_id>:dN (comma-separated for several, e.g. mse_x:d1,d4), and when THIS entry's body has 2+ decisions each item needs a 'dN -> ' prefix naming the authoring decision."},
                "evolves": {"type": "array", "items": {"type": "string"}, "description": "entry_id values this entry refines (the old decision stays valid but incomplete). Same decision-granularity grammar as replaces: <entry_id>:dN targets, 'dN -> ' source prefix when this entry is multi-decision."},
