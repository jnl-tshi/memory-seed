...
- down to the **Small work entry** for routine edits, small fixes, or verification-only work with no real decision (do not invent rationale — see Rationale Rules);
...
- A DRAFT decision record uses compact labels:
  - D = Decision (mandatory)
  - R = Rationale (mandatory)
  - A = Alternatives considered or rejected, with reason (optional)
  - F = Files, artifacts, or behaviors changed (optional)
  - T = Tests or validation (optional)
