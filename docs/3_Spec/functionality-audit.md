...
| `session merge-branch --branch <branch> [--dry-run]` | One-step branch integration: fuse dry-run gate, `git merge --no-ff --no-commit`, session-path reset, fuse apply, stage, commit, then Git-only cleanup of the exact clean registered source worktree when one exists. Fails closed before merge on fuse issues; non-session conflicts leave the merge in progress. Dirty, locked, unreadable, or partially removed worktrees are retained/reported; branches are never deleted. Requires a clean integration working tree. |
