...
- **MCP skill surface (unreleased).** `skills/index.md` routes `memory_search`/`memory_get_chunk` plus the read-only authoring-support tools (`memory_link_suggest`/`memory_link_show`/`memory_session_target`) to `history_retrieval.md`, and routes `memory_branch_status`/`memory_session_fuse_preview` to `agent_collaboration.md`. Topic MCP tools (`memory_topics_list`/`memory_topic_inspect`/`memory_topics_check`) support the same history-retrieval/session-authoring loop by letting an LLM inspect the controlled vocabulary before choosing entry topics. Every MCP tool is read-only: the authoring-support tools help an LLM fill `related_entries` and find its append target, the topic tools help it choose/validate existing topics, the collaboration tools help it diagnose branch posture and preview fuse blockers/plans, and all write application (session appends, fuse apply, topic-index edits) stays on the CLI/direct-file path with user approval where required.
