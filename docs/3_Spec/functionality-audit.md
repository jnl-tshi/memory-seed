...
- **MCP skill surface (unreleased).** `skills/index.md` routes `memory_search`/`memory_get_chunk` plus the read-only authoring-support tools (`memory_link_suggest`/`memory_link_show`/`memory_session_target`) to `history_retrieval.md`, and routes `memory_branch_status`/`memory_session_fuse_preview` to `agent_collaboration.md`. Every MCP tool is read-only: the authoring-support tools help an LLM fill `related_entries` and find its append target, the collaboration tools help it diagnose branch posture and preview fuse blockers/plans, and all write application (session appends, fuse apply) stays on the CLI/direct-file path.
...
- **MCP tools (stdio JSON-RPC):**
  ...
  - `memory_branch_status(cwd=".")` and `memory_session_fuse_preview(branch, cwd=".", base="HEAD")` -> read-only branch posture and branch-local fuse plan.
