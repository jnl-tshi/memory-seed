...
- **Reflection Board v1 foundation (merged; public launch pending).** The only supported reflection format is the sequential `memory-seed/reflection-workstream-ledger` v1 ledger at `.memory-seed/reflections/active/<workstream_id>/ledger.md`. Its trusted-history loader, Task Packet/integration admission, guarded ledger-only Git transaction writer, rebind/receipt/close primitives, and fail-closed compacted-history verification are implemented. The fragment-and-fuse prototype has no runtime reader, writer, compatibility path, or persisted-board population. Public `reflection` CLI/MCP adapters, ESR reporting, hook/runbook guidance, the first real board, and launch evaluation are **planned**; no public command should be inferred from the foundation API.
