...
date: "2026-07-07"
project: "memory-seed"
related_to: "docs/todo/memory-trace-product-and-trail-view-plan.md; docs/todo/memory-seed-explorer-distribution-plan.md; docs/todo/session-decision-diagrams-plan.md"
...
> **Proposed placement:** `docs/inbox/memory-trace-local-ai-timeline-summarisation-proposal.md` initially, promoted to `docs/todo/` only after scope and safety posture are approved.
...
Memory Seed is currently a local-first, Markdown-first control plane for AI coding agents. It stores durable memory under `.memory-seed/`, exposes retrieval through MCP, and has an optional local browser UI called Memory Lense. The README describes Memory Seed as a portable local memory system for AI coding agents, with Markdown files as the inspectable source of truth and MCP search for precise retrieval.

Memory Lense already provides a browser UI for search, filters, timeline, graph, and reader/detail views over Markdown session files, backed by a rebuildable SQLite cache outside the repository. The current architecture deliberately keeps the cache non-authoritative: Markdown remains the source of truth.
...
The repo already recognizes this gap in the decision-diagram plan. It states that a no-LLM Explorer cannot derive reasoning diagrams from prose, because turning freeform reasoning into a flowchart requires model interpretation.
...
```mermaid
flowchart TD
  A["User selects timeline / feature / entry"] --> B["Trace builds deterministic evidence pack"]
  B --> C["Evidence pack includes entries, graph edges, dates, files, commits, sidecars"]
  C --> D{"AI backend selected?"}
  D --> E["Local model adapter"]
  D --> F["Terminal agent adapter"]
  E --> G["Structured summary JSON"]
  F --> G
  G --> H["Trace validates citations against evidence pack"]
  H --> I["Render summary with entry-level citations and confidence warnings"]
  I --> J{"Export requested?"}
  J --> K["Deterministic export adapter (Docs / PPT / Excel / Miro)"]
```
  ...
  "entries": [
    {
      ...
      "summary": "...",
      "decisions": ["..."],
      "files": ["docs/todo/memory-trace-product-and-trail-view-plan.md"],
...
The initial version must not allow the AI backend to modify files. This keeps it consistent with the current Memory Lense and Memory Trace read-only direction. The distribution plan explicitly keeps Explorer/Trace read-only and says deep links should use `chunk_id`.
...
Create the proposal in `docs/inbox/`.

Acceptance criteria:

- proposal references current Memory Trace, Lense, retrieval, graph, and diagram-sidecar plans;
...
- [`docs/todo/memory-trace-product-and-trail-view-plan.md`](../todo/memory-trace-product-and-trail-view-plan.md) - Trace as product, Trail as branch/supersession evolution view.
- [`docs/todo/memory-seed-explorer-distribution-plan.md`](../todo/memory-seed-explorer-distribution-plan.md) - companion distribution split and public retrieval-service seam.
- [`docs/todo/session-decision-diagrams-plan.md`](../todo/session-decision-diagrams-plan.md) - two-class diagram model and no-LLM reasoning-diagram constraint.
- [`docs/graph-edge-contract.md`](../graph-edge-contract.md) - canonical graph/edge/metric contract.
- [`memory_seed/retrieval.py`](../../memory_seed/retrieval.py) - public MCP-independent retrieval service.
- [`memory_seed/lense.py`](../../memory_seed/lense.py) - current local read-only browser UI implementation.
