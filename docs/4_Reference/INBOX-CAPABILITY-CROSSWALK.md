...
| A1 | Decision-level identity (`dec-0042-01`) distinct from entry ID | Canonical `(entry_id, dN)` identity, resolved by the living ADR ledger and exact Trace Inspector navigation | ADR contract (live); Semantic-record plan phase 1 | narrow — shipped without introducing a second identifier namespace |
| A1 | Formal action verbs `promote`, `supersede`, `update`, `attach_evidence` | CLI `adr promote`, `adr revise`, and `adr transition`; MCP mutations remain constrained to `memory_session_append` and `memory_session_integrate` | ADR contract (live); Semantic-record plan phase 2 | narrow — ADR lifecycle actions ship; generic update and typed evidence attachment remain out of scope |
| A1 | `decisions:` block with embedded `relationships:` inside entry YAML | Decision identity and lifecycle assigned to an append-only sidecar so historical entries are never edited | ADR contract (live); Constitution invariant #2 | narrow — declined: authority for concern membership and ADR lifecycle is the sidecar, not entry YAML |
...
| A2 | Embedded `## Evidence` section (EV-01 blocks) in an ADR | Living ADR revisions reference source decisions and supporting references; session `F:` and `T:` lines remain detailed evidence | Constitution §4 DRAFT policy; ADR contract (live) | narrow — evidence is referenced, not represented as typed addressable evidence blocks |
...
| A3 | Decision-level supersession identity | Living ADR revision references use canonical `(entry_id, dN)` identities and validated `evolves`/`replaces` lineage; the entry-level collateral remains recorded in `mse_mkxdvaxvw99dz4s0` | ADR contract (live); Semantic-record plan phase 1 | none for ADR concern lineages; universal session-decision status remains intentionally separate |
...
| B2 | Four existing lenses (timeline, ADR, diagram, relationship) | Timeline entries, diagram sidecars under `sessions/diagrams/`, link sidecars under `sessions/links/`, the canonical `build_related_entry_graph` reader, and the Trace ADR workspace | Edge contract; Link sidecars; ADR contract (live) | none — all four lenses ship |
...
| B3 | Lifecycle-state vocabulary `generated`, `validated`, `reviewed`, `promoted`, `stale`, `superseded` | Authority classes distinguish `authored` from `generated`; Trace exposes `stale`; living ADR events record proposal, acceptance, rejection, no-change review, and concern supersession | Provenance taxonomy; ADR contract (live) | narrow — ADR review and promotion ship for architectural concerns, not as a universal lifecycle for every sidecar |
...
Step 2 of the corrected sequence in [INBOX-ASSESSMENT.md](INBOX-ASSESSMENT.md) was discharged on
2026-08-03 by dogfooding three architectural concerns, including multi-revision and converging lineage.
The implementation uses canonical `decision_ref` identities in the live ADR contract rather than the
retired prototype's `source_decision` pointer. Steps 3 through 5 remain undone. Step 3's
