...
`ledger.md` has a canonical header with `schema`, `version`, `workstream_id`, `working_branch`, `base_sha`,
`state`, `created_at`, `reflection_retention_days`, and an ordered phase policy. It has append-only dated
blocks. Every block contains:

- a globally stable `record_id`, a per-ledger `sequence`, and the clock-owned `created_at` timestamp;
...
`record_id` and `chain_id` are generated by the canonical writer, not supplied by callers. They are derived
from a random workstream identity plus a monotonically increasing ledger sequence, so they are unique across
ledgers without pre-reserving participant slots. The record's sequence and canonical content digest make the
identity auditable after expiry. Existing `rlr_` and `rlc_` identifiers remain valid compatibility forms;
the exact new prefix/version is frozen with the implementation schema and golden vectors.
...
enumerates valid ledgers and produces a sorted derived projection. It labels each source workstream and all
cross-ledger dependencies; it has no effective winner, consensus, merge, promotion, or write operation.
...
| --- | --- | --- |
| Ledger identity and append | Canonical init/append produces reproducible bytes, contiguous sequence, generated IDs, and a conclusion-first block. | Forged ID, manual timestamp, noncanonical bytes, duplicate sequence, stale ledger digest, stale Git tip, and direct file overwrite all refuse. |
...
| Board view | Multiple active ledgers produce a labelled, sorted, read-only combined projection. | A board command that emits a winner, writes a ledger, fuses records, or promotes a decision fails/refuses. |
