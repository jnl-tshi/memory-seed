...
10. ~~**`topics.yaml` schema v3 writer/migration — build now or park?**~~ **REJECTED 2026-08-26** (JNL):
    no migration tool needed — no other project currently deploys `memory-seed` and needs the v1/v2 → v3
    conversion, so building it has no observed benefit. Moved
    [`topics-schema-v3-writer-migration-proposal.md`](../6_Rejected/topics-schema-v3-writer-migration-proposal.md)
