...
> fan-out evaluation of a user-uploaded workflow diagram (not retained in the current docs tree), not the external
...
4. Leave the CLI scaffolding idea as a future `docs/2_Todo/` proposal only after the runbook has actually proved useful in practice.

## Status

Implemented 2026-07-03. This file supersedes the four intermediate evaluation/synthesis drafts, which were deleted once this consolidated version was written; the uploaded source image was intake-only and is not retained in the current docs tree.
