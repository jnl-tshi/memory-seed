...
status: active
priority: P1
next_action: "Implement M1 decision reader and evidence-return path against the named fixtures."
blocked_by: []
sources:
...
Status: **M0 delivered 2026-07-30.** This is the single reconciliation point for the UX reference-model plan. It records implementation truth; it does not create a second semantic or data contract.
...
| Reader | Decision-first presentation inside Inspector. | Canonical entry body, sidecars, and evidence refs. | M1 is not yet delivered. |
...
| Decision reader and exact evidence return | Parity-gap (M1) | entry body, sidecars, evidence anchor | Inspector / Reader | Follow evidence and return without losing Trail position. | Multi-decision, external, missing-source, superseded fixtures |
| Recorded / derived / suggested grammar | Parity-gap (M1) | provenance class and `source` | all surfaces | Text/icon/accessible name accompany visual treatment. | Provenance-state fixture |
...
- M1 is next: the reader/evidence-return fixtures must cover the four missing cases above before UI completion.
