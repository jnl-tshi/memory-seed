...
- `supersedes`: `superseded_by` is exposed on results and drives the dampener (`freshness-...` item 1),
  but there is **no** successor pointer and **no** replacement boost — the deliberate item-2/item-1
  split left supersedes with only the "damp" half.

## Proposal — give supersedes the successor half evolves already has

1. **`superseding_head` (surface, additive first).** Symmetric with `evolved_head`: follow the
...
- `docs/2_Todo/evolution-edges-plan.md` / `docs/2_Todo/completed/supersession-edges-plan.md` — the
