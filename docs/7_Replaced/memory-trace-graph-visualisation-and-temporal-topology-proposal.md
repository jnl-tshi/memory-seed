...
priority: "P1"
recommended_target: "docs/7_Replaced/memory-trace-graph-visualisation-and-temporal-topology-proposal.md"
related:
...
> exists only to stabilise unstable algorithmic ids, and there are none to stabilise. It is recorded
> as not required rather than outstanding, and would only be revived if detection were ever adopted.
> The one derivation that must stay single-sourced is a different concern and is already handled:
> the legend and the nodes both call `graphCommunities.ts`, because two derivations are how a legend
