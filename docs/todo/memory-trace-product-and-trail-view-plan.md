...
> **Status:** ACTIVE - promoted from inbox on 2026-07-05 as the canonical naming/product plan for
...
> **Priority:** P2a - decide before Phase 2 companion-package extraction or any public package/name
> rename; it is not a blocker for the already-implemented retrieval-service Phase 1.
...
> Prior source notes: [`../inbox/memory-trail-competitor-analysis.md`](../inbox/memory-trail-competitor-analysis.md)
...
This is explicitly framed as **an evolution of the existing Lense/Trace timeline and graph views**,
not a new product surface. Memory Lense already renders a timeline and a graph
...
- **Supersession is modeled but not rendered as a Lense graph edge.** `build_related_entry_graph()`
  computes `supersedes`/`superseded_by`; `link show` and `memory_get_chunk` expose it read-only. But
  Lense's `_graph_edges()` (`memory_seed/lense.py`) only builds `related`/`topic`/`agent`/`day` edge
...
- **Surfaced read-only** through the same paths as every other graph-edge-contract field: `link show`,
  ...
  Trace/Explorer package to consume — no new write surface, no new CLI mutation command.

## The Trail view itself

- **New Lense/Trace graph edge type: `supersedes`.** Additive to `_graph_edges()` — render the
  ...
  `docs/graph-edge-contract.md` and must never be visually conflated with plain relatedness).
...
- Retrieval/MCP/public service surfaces `branch` read-only once captured; no new write command is
  introduced.
- Lense/Trace graph work renders `supersedes` as a distinct typed edge and can group entries by
  branch labels without treating branch existence as an integrity requirement.
...
- Schema: `branch:` added alongside `commits:`/`supersedes:` in the entry YAML — documented in
  `session_logging.md`'s schema section and `docs/graph-edge-contract.md` (a new stored field,
...
- Rendering: `memory_seed/lense.py`'s `_graph_edges()` gains a `supersedes` edge type and a `branch`
  chain grouping; the dedicated Trail view is Explorer/Trace-package product work per the
  distribution plan.
...
- Conversation 2026-07-05 (JNL): the Trace/Trail naming refinement and the record-time capture
  preference.
- [`memory-trail-competitor-analysis.md`](../inbox/memory-trail-competitor-analysis.md) — the positioning-
...
- Verified directly against code: `memory_seed/semantic_cache.py` (`MemoryChunk` fields),
  `memory_seed/core.py` (`check_session_links()` entry schema), `memory_seed/lense.py`
