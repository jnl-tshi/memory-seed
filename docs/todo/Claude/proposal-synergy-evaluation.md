...
This evaluated the six active `docs/todo/` proposals (five Logic Capture items plus
`agent-fanout-workflow-plan.md`) against each other and against `docs/functionality-audit.md`, using
...
audit are accurate against real code; confirmed the audit's §14 gap (missing `agent-fanout-workflow-
...
| Audit §14 missing `agent-fanout-workflow-plan.md`, stale "4 of 5" count, stale Mermaid label | **Already fixed by Codex.** |
...
2. **`supersession-edges-plan.md`** — added a Validation-section note that the "provably acyclic"
   ...
   (`â€”` -> `—`) introduced in the open-decision note.
