...
```mermaid
flowchart TD
    P[".memory-seed/project.yaml"] --> D["sessions/2026-06-13/"]
    D --> J["jean.memory.md<br/>hash_id = msm_..."]
    D --> A["amina.memory.md<br/>hash_id = msm_..."]
    J --> E1["entry_id = mse_..."]
    J --> E2["entry_id = mse_..."]
    A --> E3["entry_id = mse_..."]
    J -. related_memories .-> A
    E2 -. related_entries .-> E3
    M["indexes/manifest.jsonl or index.db"] --> J
    M --> A
    M --> E1
    M --> E2
    M --> E3
...
```mermaid
flowchart LR
    A["Legacy flat files<br/>sessions/YYYY-MM-DD.md"] --> B["Dual-read parser<br/>legacy + new layout"]
    B --> C["Backfill file hash_id<br/>preserve existing entry_id"]
    C --> D["Split legacy files by user_initials<br/>into YYYY-MM-DD/{user}.memory.md"]
    D --> E["Build manifest/index<br/>path + IDs + line ranges"]
    E --> F["Switch writers and hooks<br/>to per-user/day"]
    F --> G["Remove legacy write path<br/>keep legacy read fallback briefly"]
```
