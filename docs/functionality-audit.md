...
- **Multi-user per-day session memory - completed** (`docs/todo/completed/multi-user-session-memory-proposal.md`):
...
- **Related-entries generation P2 (approved 2026-07-05; active plan)** (`docs/todo/related-entries-p2-mutation-plan.md`): P1 (`link suggest`/`link show`/`build_related_entry_graph()`, §3I) shipped in 2.13.0. P2 now covers controlled `link add` for current/newest-entry links and explicit historical backfill for two pre-existing entries, with backups, idempotency, and `links check` validation. It is sequenced after the lower-risk retrieval/diagram/risk-signaling work unless reprioritized.
- **Pillar B separate-distribution — decided 2026-07-05, plan active** (`docs/todo/memory-seed-explorer-distribution-plan.md`): Memory Lense shipped in 2.13.0 (§3O) as an in-package optional extra, delivering the local read-only web UI, explainability fields, and rebuildable cache the original Pillar B spec called for — as a UI **prototype vehicle**. The decision is now made: spin the UI out into a separate companion package (core stays a lightweight control plane; the UI iterates on its own cadence), with the in-package `memory-seed[lense]` extra going maintenance-only. The intended product name is Memory Trace; Trail is the internal evolution view. Target package/command is `memory-seed-trace` unless PyPI + web/trademark sanity checks show a problem. **Phase 1 is implemented (2026-07-05, unreleased):** the public retrieval service (`memory_seed/retrieval.py`, §3I) exists with MCP as a parity-tested thin wrapper, Lense consuming the service, the entry-level rollup contract, and decision-diagram surfacing. **Phase 2 (package extraction) is gated on the Trace availability check and explicit go** - see the Memory Trace product plan below. The source research report moved to `docs/todo/completed/user-interface-deep-research-report.md` (citation artifacts scrubbed 2026-07-05).
...
- **Seed skill promotion: Windows DOCX render fallback — implemented 2026-07-05 (unreleased)** (`docs/todo/completed/docx-render-windows-seed-lessons.md`): the universal lazy skill `docx_render_windows.md` (LibreOffice profile-URI failure mode, bounded two-step render, stale-process cleanup, Word field refresh, page-level visual QA, single-writer render boundary) ships in the seed, registered in the trigger registry and wired into `SEED_FILES`/package data/tests.
...
- **Memory Explorer entry-level UI results — core contract implemented 2026-07-05 (unreleased)** (`docs/todo/memory-explorer-entry-level-ui-results-plan.md`): session entries are the selectable result object; the shared `EntryRollup` contract (§3I) collapses section-chunk matches into one entry-level result with `best_match_chunk_id`/`matched_sections`/`score_source`, Lense entry-granularity search consumes it, section/all granularities and MCP stay unchanged, and Lense copy uses entry/"session entry" with "Matched section" chips. Remaining for the Trail UI pass: reader-view scroll-to/highlight of the best-matching subsection.
- **Memory Trace product + Trail view - active** (`docs/todo/memory-trace-product-and-trail-view-plan.md`): the Trail-only product name is superseded. Memory Trace is the companion package/product direction; Trail is the internal feature-evolution view combining recorded branch labels, supersession edges, commit links, related entries, and diagrams. Phase 0 remains: run the Trace availability check before extraction.
- **Risk signaling and STOP triggers - implemented 2026-07-05 (unreleased)** (`docs/todo/completed/risk-signaling-and-stop-triggers-plan.md`): consolidates the confidence-signaling and STOP-trigger inbox research into one seeded lazy-loaded guidance skill before higher-risk mutation or automation work. Scope remains behavioral guidance only: no numeric confidence field, no emoji protocol, no new session schema, and no automated gate.
- **Session decision diagrams - Phase 1 implemented 2026-07-05 (unreleased)** (`docs/todo/session-decision-diagrams-plan.md`): the two-class diagram model's Class-2 sidecar convention is built (§3I, §3J) - `sessions/diagrams/YYYY-MM-DD.md` sidecars (one dated file per day, mirroring session-log filenames; each diagram a heading block naming its `entry_id`) validated by `links check` (malformed/orphan/date-mismatch), surfaced through the retrieval service and Lense chunk metadata, with authoring guidance in `session_logging.md`/`end_of_turn.md` (live + seed) and a first real sidecar in this repo. Phase 2 (Explorer rendering) and Phase 3 (the exportable report/handover pack - the paid deliverable, which still does not exist) remain gated as scoped.
...
graph LR
  ...
  V30["Next — related-entries P2 · Trace package + risk signaling"]
