...
- **Logic Capture Improvements — 2 of 6 implemented (unreleased), 4 still proposed** (sourced from an external review doc, refined 2026-07-02/03): implemented — [`mermaid-usage-guidance-plan.md`](todo/completed/mermaid-usage-guidance-plan.md) and [`failed-approaches-logging-plan.md`](todo/completed/failed-approaches-logging-plan.md) (§3D). Still proposed — [`git-commit-entry-linking-plan.md`](todo/git-commit-entry-linking-plan.md) (link a decision entry to the commit(s) that implemented it; trailer key signed off as `Memory-Entry:`), [`supersession-edges-plan.md`](todo/supersession-edges-plan.md) (a typed `supersedes` edge, with a harmony contract against naive importance scoring; forward-only/cycle guard in P1 scope), [`interaction-frequency-ranking-plan.md`](todo/interaction-frequency-ranking-plan.md) (raw `related_degree` first; supersession-aware `importance_score` only after `supersedes`; access-frequency telemetry deferred), and [`exclude-superseded-filter-plan.md`](todo/exclude-superseded-filter-plan.md) (opt-in `memory_search` narrowing filter, blocked on `supersedes` P1). Priority order is P0 roadmap/shared contracts, P1 low-risk guidance plus graph semantics, P2 read-only metadata surfacing, P3 mutation/telemetry automation.
