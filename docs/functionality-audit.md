...
- **Integrity validation (new in 2.12).** `check_session_links()` / `memory-seed links check` scans for duplicate `entry_id`/`hash_id` and dangling refs, exiting non-zero as a CI gate. The legacy-flat `related_entries` scan gap was fixed in 2.14. Unreleased after 2.14: `links check` also validates `supersedes` refs (dangling/self/postdates/cycle) and `commits:` hashes (malformed/unknown when git is present). Unreleased after 2.15: it also validates decision-diagram sidecars under `sessions/diagrams/` (`orphan-diagram`, `diagram-filename-mismatch`, `malformed-diagram`; sidecars always optional).
- **Decision diagram sidecars (new, unreleased — diagrams plan Phase 1).** Optional authored reasoning diagrams at `sessions/diagrams/<entry_id>.md` (frontmatter `entry_id` + fenced mermaid blocks), keyed to one entry and never inlined in the append-only log. Authoring guidance (same spatial/temporal/concurrent bar as the Mermaid Working Principle) lives in `session_logging.md` + `end_of_turn.md` (live + seed); metadata surfaces through `retrieval.entry_diagram_sidecars()` and Lense's chunk view.
...
and the seeded `docx_render_windows.md` skill (Windows DOCX render fallback) joined the seed inventory
...
- **Memory Trail rename — decision needed** (`docs/todo/memory-trail-renaming-plan.md`): the Phase-0 availability check (2026-07-05) found `memory-seed-trail` free on PyPI but **"Memory Trail" in active use by a same-niche competitor** (decision memory + session logging for AI-assisted development, on PyPI and LobeHub). The docs rename is paused until the user confirms or changes the product name; `lense` remains the deprecated-alias path either way.
- **Session decision diagrams — Phase 1 implemented 2026-07-05 (unreleased)** (`docs/todo/session-decision-diagrams-plan.md`): the two-class diagram model's Class-2 sidecar convention is built (§3I, §3J) — `sessions/diagrams/<entry_id>.md` sidecars validated by `links check` (orphan/filename-mismatch/malformed), surfaced through the retrieval service and Lense chunk metadata, with authoring guidance in `session_logging.md`/`end_of_turn.md` (live + seed) and a first real sidecar in this repo. Phase 2 (Explorer rendering) and Phase 3 (the exportable report/handover pack — the paid deliverable, which still does not exist) remain gated as scoped.
