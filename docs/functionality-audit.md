...
- **Pillar B separate-distribution — decided 2026-07-05, plan active** (`docs/todo/memory-seed-explorer-distribution-plan.md`): Memory Lense shipped in 2.13.0 (§3O) as an in-package optional extra, delivering the local read-only web UI, explainability fields, and rebuildable cache the original Pillar B spec called for — as a UI **prototype vehicle**. The decision is now made: spin the UI out into a separate companion package (core stays a lightweight control plane; the UI iterates on its own cadence), with the in-package `memory-seed[lense]` extra going maintenance-only. The intended product name is Memory Trail, governed by `docs/todo/memory-trail-renaming-plan.md`; `memory-seed-explorer` remains a working placeholder until package/command availability checks complete. The scoped two-phase plan — Phase 1 freezes a public retrieval service in-package (extracting `call_tool`/`format_search_results`/`_ranked_to_dict`/`_chunk_to_dict`/`_semantic_provider` from `mcp_server.py` alongside the shared `semantic_cache.py` parser/ranker); Phase 2 extracts the distribution — is P2 priority and not a blocker for any shipped surface. The source research report moved to `docs/todo/completed/user-interface-deep-research-report.md` (citation artifacts scrubbed 2026-07-05).
...
- **Memory Trail rename - proposed** (`docs/todo/memory-trail-renaming-plan.md`): rename the future Explorer/Lense UI product line to Memory Trail before publishing the companion package; keep `lense` as a deprecated alias during the transition and check package/trademark availability before release-facing changes.
