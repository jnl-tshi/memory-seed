...
- **Pillar B separate-distribution — decided 2026-07-05, plan active** (`docs/todo/memory-seed-explorer-distribution-plan.md`): Memory Lense shipped in 2.13.0 (§3O) as an in-package optional extra, delivering the local read-only web UI, explainability fields, and rebuildable cache the original Pillar B spec called for — as a UI **prototype vehicle**. The decision is now made: spin the UI out into a separate companion package (core stays a lightweight control plane; the UI iterates on its own cadence), with the in-package `memory-seed[lense]` extra going maintenance-only. The intended product name is Memory Trail; target package/command is `memory-seed-trail` unless PyPI + web/trademark sanity checks show a problem. The scoped two-phase plan — Phase 1 freezes a public retrieval service in-package (extracting `call_tool`/`format_search_results`/`_ranked_to_dict`/`_chunk_to_dict`/`_semantic_provider` from `mcp_server.py` alongside the shared `semantic_cache.py` parser/ranker), adds entry-level rollup, and surfaces decision-diagram sidecars; Phase 2 extracts the distribution — is P2 priority and not a blocker for any shipped surface. The source research report moved to `docs/todo/completed/user-interface-deep-research-report.md` (citation artifacts scrubbed 2026-07-05).
...
- **Memory Explorer entry-level UI results - proposed** (`docs/todo/memory-explorer-entry-level-ui-results-plan.md`): the Explorer UI should make session entries the selectable result/node/event object and highlight high-scoring subsection matches inside those entries, rather than presenting subsections as separate frontend records. This proposal connects the Explorer split plan to the UI source-learnings note on hierarchy, navigation clarity, component consistency, and microcopy.
- **Memory Trail rename - proposed** (`docs/todo/memory-trail-renaming-plan.md`): rename the future Explorer/Lense UI product line to Memory Trail before publishing the companion package; target `memory-seed-trail` for package/command unless availability checks show a problem; keep `lense` as a deprecated alias for at least one release during the transition.
- **Session decision diagrams - proposed** (`docs/todo/session-decision-diagrams-plan.md`): a two-class diagram model for non-technical stakeholder review. Class 1 (structural) — provenance/timeline/graph views the no-LLM Explorer derives deterministically from `build_related_entry_graph()`, plus a new exportable static report/handover pack (the paid deliverable, which does not exist yet). Class 2 (reasoning) — per-decision flow/sequence diagrams the agent authors at entry time as `.memory-seed/sessions/diagrams/<entry_id>.md` sidecars (a no-LLM Explorer cannot derive these from prose), kept out of the append-only log, validated by `check_session_links` (orphan/filename-mismatch/malformed) and rendered beside their entry. Phase 1 (convention + validation + authoring trigger) is core-side and Explorer-independent; Phase 3 (report pack) is gated on the Explorer split.
