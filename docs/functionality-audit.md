...
**Unreleased, queued under `CHANGELOG.md`'s `## Unreleased`:** the `links check` legacy-flat validation fix (§3J); participant-count session-layout gating plus the one-time SessionStart identity offer and the `doctor` local-user/participant-mismatch warning (§3D, §3H, §3J, §3L); and two new Working Principles bullets (decision ladder, guard-preservation) backed by regression tests for the two guards a codebase audit found genuinely untested (§3D, §15). No control-plane version bump for any of these yet.
...
- **Logic Capture Improvements — 5 of 5 still proposed, not yet decided or built** (sourced from an external review doc, refined 2026-07-02): [`git-commit-entry-linking-plan.md`](todo/git-commit-entry-linking-plan.md) (link a decision entry to the commit(s) that implemented it), [`supersession-edges-plan.md`](todo/supersession-edges-plan.md) (a typed `supersedes` edge, with a harmony contract against naive importance scoring), [`interaction-frequency-ranking-plan.md`](todo/interaction-frequency-ranking-plan.md) (raw `related_degree` first; supersession-aware `importance_score` only after `supersedes`; access-frequency telemetry deferred), [`mermaid-usage-guidance-plan.md`](todo/mermaid-usage-guidance-plan.md), and [`failed-approaches-logging-plan.md`](todo/failed-approaches-logging-plan.md). All shared or partly shared a discovered dependency on the `links check` legacy-flat gap (§3J, §12) — now fixed, so all are unblocked. Priority order is P0 roadmap/shared contracts, P1 low-risk guidance plus graph semantics, P2 read-only metadata surfacing, P3 mutation/telemetry automation.
- **Agent Collaboration Workflow — proposed, not yet decided or built** (`docs/todo/agent-fanout-workflow-plan.md`): adds a vendor-neutral fanout recipe for `agent_collaboration.md`, with explicit Level 2/3 justification, handoff-only worker agents, orchestrator-only shared-file writes by default, and a bounded review-to-rework loop capped at 2 iterations. Any future CLI scaffold is an evaluation loop only and must be preview-first/dry-run; it should not spawn agents, mutate branches, or edit shared memory directly.
