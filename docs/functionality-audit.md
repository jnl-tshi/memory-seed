...
**As of:** 2026-07-03 · control-plane `2.13` · package `2.13.0`
...
- **Integrity validation (new in 2.12).** `check_session_links()` / `memory-seed links check` scans for duplicate `entry_id`/`hash_id` and dangling `related_entries`/`related_memories` refs, exiting non-zero as a CI gate. **Gap found and fixed (2026-07-02, unreleased):** the entry-level `related_entries` scan used to only run inside the per-user-day branch, silently skipping legacy-flat `sessions/YYYY-MM-DD.md` files — this repository's own layout. Fixed by moving the entry-level (fenced `` ```yaml `` block) scan out of the per-user-day gate, since it parses the same shape in both layouts; the genuinely per-user-file-specific checks (frontmatter, `hash_id`, user-slug) remain correctly gated. Verified with a regression fixture and confirmed clean against this repo's own history.
...
| `links check` skipped legacy-flat `related_entries` validation | A dangling `related_entries` ref in this repo's own layout wasn't caught | **Fixed (2026-07-02, unreleased)** — entry-level scan moved out of the per-user-day-only gate; regression-tested |
...
**Unreleased, queued under `CHANGELOG.md`'s `## Unreleased`:** the `links check` legacy-flat validation fix (§3J); participant-count session-layout gating plus the one-time SessionStart identity offer and the `doctor` local-user/participant-mismatch warning (§3D, §3H, §3J, §3L); two new Working Principles bullets (decision ladder, guard-preservation) backed by regression tests for the two guards a codebase audit found genuinely untested (§3D, §15); the Mermaid usage guidance bullet and the failed-approaches logging rule (§3D); and the Fan-Out Recipe + task-packet field additions in `agent_collaboration.md` (§3F).
...
- **Logic Capture Improvements — 5 of 6 implemented (unreleased), 1 still proposed** (sourced from an external review doc, refined 2026-07-02/03): implemented — [`mermaid-usage-guidance-plan.md`](todo/completed/mermaid-usage-guidance-plan.md) and [`failed-approaches-logging-plan.md`](todo/completed/failed-approaches-logging-plan.md) (§3D); [`supersession-edges-plan.md`](todo/supersession-edges-plan.md) P1 (typed `supersedes` edge, computed `superseded_by`, `links check` guards incl. forward-only/cycle, `link show`/`memory_get_chunk` exposure — harmony-contract dampening still pending with ranking P1b); [`git-commit-entry-linking-plan.md`](todo/git-commit-entry-linking-plan.md) P1 (`Memory-Entry:` trailer, `commits:` field, git-gated validation, `memory-seed link commits`); and [`interaction-frequency-ranking-plan.md`](todo/interaction-frequency-ranking-plan.md) P1a + P1b (raw `inbound_relation_count` and supersession-aware `importance_score` — dampened ×0.25 when superseded — exposed read-only; Lense's separate combined-degree metric renamed to `connectivity`; access-frequency telemetry deferred). Still proposed — [`exclude-superseded-filter-plan.md`](todo/exclude-superseded-filter-plan.md) (opt-in `memory_search` narrowing filter, now unblocked since `superseded_by` exists). Priority order is P0 roadmap/shared contracts, P1 low-risk guidance plus graph semantics, P2 read-only metadata surfacing, P3 mutation/telemetry automation.
- **Agent Collaboration Workflow — implemented 2026-07-03 (unreleased)** (`docs/todo/completed/agent-fanout-workflow-plan.md`): the vendor-neutral Fan-Out Recipe now lives in `agent_collaboration.md` (§3F) — explicit Level 2/3 justification, handoff-only worker agents, orchestrator-only shared-file writes by default, preflight identity verification for both writers and read-only explorers, and a bounded review-to-rework loop capped at 2 iterations. Any future CLI scaffold remains an evaluation loop only and must be preview-first/dry-run; it should not spawn agents, mutate branches, or edit shared memory directly.
...
graph LR
  ...
  V30["3.0 — proposal priority order · graph contracts · fanout/render skill proposals"]
...
- `tests/test_memory_seed.py` (init/update/doctor/agents/hooks/MCP-merge, seed-file list, version lockstep, orphan-skill + route-presence warnings, foreign-routing merge, user-aware session targets, links check, migrate sessions-layout, link suggest/show, participant-count session-layout gating, the SessionStart identity-offer hook, guard-regression tests for `_valid_session_date` and the MCP ownership-preservation check, and a new full-text seed/live parity test for `agent-rules.md`) and `tests/test_session_schema.py` (session frontmatter, seed/live parity, public-docs contract, docs/todo completeness checks). Current suite: **219 tests** (verified 2026-07-03).
