...
| Reflection Board evidence (historical) | The former tracked ledger (Git blob `e65638b9dcf682d288773346cd5968093ad70399`) and ordinary session decisions `mse_qrh0wtg81prbfqmn:d1` and `mse_awe4krg10x9t2acs:d1` record the closed v1 chain. The [retirement record](reflection-runtime-retirement-superpowers-plan.md) describes the later governed disposition. | This row's original instruction to preserve the active ledger and code was superseded by JNL's Constitution v2.1 approval. Use the session evidence as historical context, not as a current acceptance scenario or active-board requirement. |
