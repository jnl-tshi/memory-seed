...
completion_note: "Decision diagram storage, validation, retrieval, and rendering shipped; optional report and handover packaging is parked separately."
...
> (exportable report/handover pack) remains gated as scoped below.
...
> tool (see the market-fit visual appendix), but a **no-LLM Memory Trace cannot derive a reasoning diagram
...
> provenance/timeline/graph views Memory Trace computes deterministically from the graph-edge contract,
...
> Memory Trace alongside their entry.
> **Non-goals:** No LLM/NLP in Memory Trace or in `links check` (deterministic only). No requirement
...
differentiator. The other naive fix - "have Memory Trace generate diagrams from the prose at render
time" - is impossible for a **no-LLM, local-first, deterministic** Memory Trace: you cannot turn freeform
...
`build_related_entry_graph()`. A no-LLM Memory Trace computes them deterministically at render time, so
they are **uniform** (every decision gets the same treatment) and **always current** (a
`superseded_by` badge appears automatically). Interactive versions already exist in Memory Trace
...
The two compose: a superseded decision keeps its frozen Class-2 flow diagram while Memory Trace
overlays a live Class-1 `superseded_by` badge - accurate without editing the frozen asset.

## Phase 1 - Sidecar convention, validation, and the authoring trigger (core, Trace-independent)

Buildable now, independent of the Trace split. This is the part that determines whether the folder
...
- Location: `.memory-seed/sessions/diagrams/YYYY-MM-DD.md` - **one file per date**, mirroring the
  ...
  date-named files maximize human readability when visually traversing the raw Markdown without
  Memory Trace - a user can open the day's diagrams file next to that day's session log and match by
...
- File shape: append a heading block shaped like a session entry - `## <timestamp> - <title>`,
  ...
  of truth stays Markdown/text, diffable and git-native; Memory Trace extracts the fenced block(s) and
...
- Live Class-1 provenance is available through the shared graph/Trail surfaces (`superseded_by`,
  inbound/outbound edges, commit links).
- Read-only; entry remains the canonical deep-link target (per the entry-level UI results plan).

### Phase 2 acceptance criteria

- **Implemented 2026-07-06:** opening an entry with a supported sidecar renders the diagram; opening
  one without a sidecar renders normally.
- **Implemented 2026-07-06:** unsupported diagram types render escaped source instead of a blank
  frame.
