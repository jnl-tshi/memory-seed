...
> the foundation is the derived-projection Phase 1.
...
Updated 2026-07-14 after an inbox+todo evaluation, a strategy-doc triage, the Constitution-first pause,
promoting the ranking/graph-quality trio (inbox → todo), and approving+shipping the grounding-provenance
write-time links proposal (the `consulted` link axis) → `5_Completed`.
Inbox 2 · Todo 21 (incl. `0_NEXT_STEPS.md`; development resumed Constitution-aligned; +2 persona
context-efficiency proposals; +3 promoted ranking/graph-quality proposals) · Completed 7 (+43
still in `2_Todo/completed/`, pending the P2 bulk migration) · Superseded 2 · Deferred 2 · Spec 5 live +
3 candidate (incl. the derived-projection contract) · Reference 13 · plus top-level `CONSTITUTION.md`
(ratified).
