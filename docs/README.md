...
This front door and the lane folders are **Phase 1** of that plan. The Phase 2 bulk migration is **done**
(2026-07-17): the legacy `2_Todo/completed/` archive moved to `5_Completed/` and that folder is retired,
so every terminal doc now lives in a lane. The `memory-seed docs check` / `docs index` tooling is the
