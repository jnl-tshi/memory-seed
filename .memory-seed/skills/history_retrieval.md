...
- `memory_session_append`: append a session entry with every structural guard enforced — chronology, ref existence (fabricated ids refused), forward-only `replaces`/`evolves` edges, controlled topic vocabulary, id collision, and DRAFT body format. This is the **only** sanctioned way to author an entry; do not hand-write session files. The server stamps the heading timestamp from its own clock — omit `timestamp` in normal use (an explicit far-off value earns a drift warning). Refusals come back as `{ok: false, issues: [...]}`, each independently fixable. Required: `title`, `body`, `user_initials`, `agent_type`; common options are `topics`, `related_entries`/`replaces`/`evolves`, `branch`, and `dry_run`. A `dry_run` runs every guard and returns the `entry_id`, timestamp, target path, and `rendered` — the exact entry block a real call would append, so the final output is inspectable **before writing** — the id/target pre-flight the removed `memory_entry_id`/`memory_session_target` tools used to provide. To commit to the previewed bytes, pass the previewed `timestamp` back on the real call: the id hashes the timestamp, so a fresh server stamp that ticks to the next minute mints a different id than the one inspected. Choose the edges with `memory_link_suggest`/`memory_link_show` above first.
