...
- Companion review UI (separate distribution): `memory-trace/` â€” the `memory-trace` package/command (formerly the in-package Memory Lense). Depends on `memory-seed`, consumes `memory_seed/retrieval.py`, and owns the web stack (`fastapi`/`uvicorn`) + static assets (`memory-trace/memory_trace/static/`). Core ships no web framework; `memory-seed lense` is a deprecation shim pointing here.
- Seed templates: `memory_seed/seed/`.
- Tests: `tests/`.
- Public docs: `README.md`, `CHANGELOG.md`, `docs/2_Todo/NEXT_STEPS.md`.
...
- `.claude/settings.json` and `.codex/hooks.json` are handled as JSON merge targets during init/update Ã¢â‚¬â€ not seed file copies Ã¢â‚¬â€ so existing agent config is preserved.
...
- Claude Code reads project-scope MCP servers from a project-root `.mcp.json`, NOT from `.claude/settings.json > mcpServers` (silently ignored). Versions 2.2.0Ã¢â‚¬â€œ2.3.0 mis-wrote it to settings.json; `update` now writes `.mcp.json` and strips the dead block (ours-only).
...
- Orphan/dead-artifact review (2.7.0): the End Of Turn routine (`agent-rules.md` + seed twin, mirrored in `.claude/commands/esr.md`) gained a diff-scoped **orphan & artifact sweep** Ã¢â‚¬â€ confirm new files/features are wired in, grep for references left dangling by deletions/renames, flag scratch debris; an optional declared dead-code tool (vulture/ruff, knip, ArchUnit, cppcheck) may be run but is never installed. The sweep catches orphan *files/features*; whole-codebase *dead code* stays a periodic tool job. Backstopped by a deterministic `doctor` warning: any `.memory-seed/skills/*.md` not registered in `skills/index.md` is flagged as an orphan skill.
- Non-destructive routing into foreign entry-point files (2.8.0): the four routing files (`AGENTS.md`, `CLAUDE.md`, `GEMINI.md`, `.github/copilot-instructions.md`, set `ROUTING_DESTINATIONS` in `core.py`) follow a 4-way ownership branch in `init`/`update`: greenfield Ã¢â€ â€™ write full seed file; ours (has `memory-system-version` frontmatter) Ã¢â€ â€™ version-gated archive+replace; foreign with our markers Ã¢â€ â€™ re-sync the managed block in place; foreign without markers Ã¢â€ â€™ inject a marker-delimited routing block (`_merge_routing_stanza`, mirrors `_merge_grouped_hook`). A foreign file is **never overwritten** Ã¢â‚¬â€ even under `init --force`. The "second merge" on a version bump replaces only the block, gated on content-equality (no churn when stanza text is unchanged). **Behavior change:** a versionless entry-point file is now merged, not overwritten (retired the legacy unversionedÃ¢â€ â€™clobber path). Backstopped by a `doctor` warning: a `.memory-seed/` runtime with a present-but-foreign-without-block entry-point file is flagged as an orphaned runtime.
...
- Memory Lense V1 (current unreleased worktree): `memory-seed lense` serves an optional local FastAPI/Uvicorn UI backed by a rebuildable SQLite cache outside the repo by default. Markdown session files remain the source of truth; the UI exposes searchable memory, filters, timeline, graph, reader/details, pane resizing, light/dark themes, and accent palettes through paged APIs. Browser-verified behavior includes all-chunk graph, filter propagation, graph node selection into the right pane, sticky horizontally-scrollable timeline overview, empty-bucket toggle, and bucket navigation without mutating date filters.
- ESR generalization (2.11.0): the "End Of Turn" routine in `agent-rules.md` (+ seed twin) now runs a consolidation review (promote durable facts â†’ `index.md`/`policy.md` via `memory_consolidation`) and a baseline-promotion check (flag generic adaptations, record in `.memory-seed/plans/`, create-if-needed). Shipped as a seeded `/esr` command via two `SeedFile`s: `.claude/commands/esr.md` (agent=claude, version-tracked frontmatter, refreshes on update) and `.gemini/commands/esr.toml` (agent=gemini, deploy-once via `_is_runtime_local_file` since TOML carries no version marker). Codex/Cursor run the routine from `agent-rules.md`. No blocking `Stop` hook (deliberate â€” evolution needs reasoning + approval).
