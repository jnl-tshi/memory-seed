...
6. `docs/CONSTITUTION.md` — this project's ratified constitution (currently **v1.3**, seven
   invariants). It is the highest authority for what may change and what may not; weigh any
...
- Decision identity is `(entry_id, dN)` and the Trail renders it: the entry row anchors as a heading with D1..DN as pastel subheading rows. Link-sidecar refs remain ENTRY-to-entry only — decision-level refs are drafted in `docs/3_Spec/draft/decision-level-link-sidecar-refs.md` and NOT implemented, so a `#decisions/`-suffixed ref written today silently truncates to the entry id.
