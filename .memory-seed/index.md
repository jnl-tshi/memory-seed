...
- `model2vec` is a REQUIRED dependency and the package's only one; `pip install --no-deps memory-seed` is the supported lightweight install without semantic ranking. When the provider cannot load, ranking degrades to lexical silently — `memory-seed esr` reports this on every run.
...
- Indexed topics (completed 2026-07-15; **redesigned into a two-axis hierarchy 2026-07-26/27**): `topics:` is
  ...
  only if it fits every ancestor. **The topic sidecar is now the topic authority, not a derived overlay**: a
