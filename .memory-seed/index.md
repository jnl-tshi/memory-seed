...
- Goal run 2026-07-10 COMPLETE: all four phases of `docs/2_Todo/completed/goal-roadmap-refinement-and-staged-implementation.md` executed (alignment audit, consolidation, review swarm, stages S2-S6); v2.17.0 released; stage-to-merge-commit map in `docs/2_Todo/0_NEXT_STEPS.md`. Open follow-ups: Memory Trace packaging fold-in to `memory-seed[trace]` (revised 2026-07-11 after PyPI blocked `memory-trace` as too similar to `memorytrace`), topics plan Phase 4 (Trace rendering + MCP tools), Trace lineage pass, README screenshot captures.
...
- Companion review UI (Trace source / optional extra target): `memory-trace/` — the `memory_trace` UI source package and `memory-trace` command (formerly the in-package Memory Lense). It consumes `memory_seed/retrieval.py` and owns the web stack (`fastapi`/`uvicorn`) + static assets (`memory-trace/memory_trace/static/`). The release strategy now folds this into the root `memory-seed[trace]` install path rather than a separate PyPI project; plain `memory-seed` must still ship no web framework, and `memory-seed lense` remains a deprecated shim/alias path.
...
- Authoring-loop MCP tools (current unreleased worktree): `mcp_server.py` adds three read-only tools so the LLM authoring loop matches the retrieval loop — `memory_link_suggest` and `memory_link_show` wrap `suggest_related_entries`/`build_related_entry_graph` (paste-ready `related_entries` candidates and graph-node traversal), and `memory_session_target` resolves the append target (`create=False`, never writes). All three are documented under `history_retrieval.md`'s authoring-support section, keeping branch/fuse coordination in `agent_collaboration.md`. The CLI `session` group help was relabeled to cover the write-capable `fuse` subcommand, and the two `migrate` subcommands now cross-reference each other.
