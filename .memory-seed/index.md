...
- Multi-user session dual-read discovery (2.9.0): package readers use `iter_session_documents()` in `core.py` to read both legacy flat session files (`sessions/YYYY-MM-DD.md`) and per-day/per-user files (`sessions/YYYY-MM-DD/<user>.md`, bare slug such as `jean.md`). Phase 1 is read-only: `extract_memory_chunks()`/MCP and `compact_sessions()` discover both layouts, fallback chunk IDs use date-qualified source paths, and writes/hooks/user resolution/migration remain deferred.
