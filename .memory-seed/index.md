...
- Goal run 2026-07-10 COMPLETE: all four phases of `docs/2_Todo/completed/goal-roadmap-refinement-and-staged-implementation.md` executed (alignment audit, consolidation, review swarm, stages S2-S6); v2.17.0 released; stage-to-merge-commit map in `docs/2_Todo/0_NEXT_STEPS.md`. Memory Trace packaging was revised and implemented 2026-07-12 as the root-package `memory-seed[trace]` extra with the `memory-trace` command. Open follow-ups: topics plan Phase 4 (Trace rendering + MCP tools), README screenshot captures, `/api/v1` port of the Trail merge/lifecycle surface, MCP-graph sidecar-edge parity. The Trace lineage pass was substantially delivered 2026-07-12 (commit-accurate merges + lifecycle routes in the Trail).
- Memory Trace next-generation planning promoted 2026-07-11: `docs/2_Todo/memory-trace-product-and-system-architecture-blueprint.md` is the top-level plan, `docs/2_Todo/memory-trace-next-generation-implementation-roadmap.md` sequences future work, and `docs/2_Todo/memory-trace-next-generation-coverage-matrix.md` preserves which older implementation plans remain active.
...
- Indexed topics P1 (current unreleased worktree): `topics:` is a first-class 1-3-slug entry field resolved against the deploy-once project-local `.memory-seed/topics.yaml` (this repo: 19 canonical topics + aliases derived from the slugs already in use, user-approved; seed: minimal generic starter). `MemoryChunk.topics` + retrieval exposure + alias-expanded opt-in `memory_search` topics filter + `memory-seed topics list`/`check` (separate from `links check` by design) + `session_logging.md` authoring guidance (live + seed). Trace indexed-topic rendering and MCP topic tools deferred (plan Phase 4).
- Evolution edges and artifact lineage (current unreleased worktree): typed `evolves:` edge with read-time-only `evolved_by` inverse (no importance dampener - freshness, not retirement), `authored-inverse-field` guard enforcing that inverses are never stored (append-only), structured `continuity:` blocks (rename/migration/removal with from/to, `malformed-continuity` validation) whose recorded mappings feed a transitive alias table, rarity-weighted `F:` file-overlap boost in `link suggest`/`memory_link_suggest` with shared-file evidence, computed `superseded_by`/`evolved_by` on `memory_search` results (D7 - additive, order untouched), and Decision Harvest lifecycle + continuity prompts in `session_logging.md`/`end_of_turn.md` (live + seed). Spec updated in `docs/3_Spec/graph-edge-contract.md`; plan `docs/2_Todo/evolution-edges-plan.md`.
...
- Memory Trace UI (current unreleased worktree): the human review UI has a separate source boundary under `memory-trace/`, but its public install target is `memory-seed[trace]` with the `memory-trace` command. The old `memory-seed lense` route is a deprecated compatibility shim. Markdown session files remain the source of truth; Trace exposes searchable memory, filters, graph, Trail (primary view: git-graph timeline with branch lanes, commit-accurate merges from Memory-Entry trailers with an estimated fallback, trunk merge rings, phantom trunk, typed lifecycle routes, on-device worktree switcher), reader/details with Authored-in/Merged-by split, pane resizing, light/dark themes, accent palettes, entry-level results with section highlights, and client-side Mermaid sidecar rendering through paged APIs. Assets are content-hash versioned at serve time and `--static-root` serves another checkout's UI. New UI surface lands legacy-`/api/*`-first; `/api/v1/*` follows once polished.
- Session-authoring and end-of-turn tooling (current unreleased worktree, 2026-07-12/13): lifecycle-edge **link sidecars** (`sessions/links/YYYY-MM/YYYY-MM-DD.md`, append-only late edges validated by `links check` and rendered by the Trail; spec `docs/3_Spec/lifecycle-edge-linking-sidecars.md`), `memory-seed link audit` (file/topic-overlap gap finder feeding the end-of-turn Lifecycle Link Sweep), `session append` (canonical structured entry authoring), `session reorder` (chronology repair as pure block permutation), `session entry-id` + `memory_entry_id` (deterministic ids, never hand-rolled), `memory-seed esr` (one-command mechanical end-of-turn preflight), and the `prepare-commit-msg` trailer hook. Rationale recorded in `docs/2_Todo/completed/cheap-tooling-hardening-proposals.md`: a skill rule without a one-call tool does not hold against flow.
